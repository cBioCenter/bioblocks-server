...
def average_profile(E, all_genes, gene_set):
    ...
    gix = np.array([i for i, g in enumerate(all_genes)
                    if g.upper() in gene_set], dtype=int)
    if len(gix) == 0:
        return np.zeros(E.shape[0])
    else:
        gix = gix[E[:, gix].sum(0).A.squeeze() > 0]
        if len(gix) == 0:
            return np.zeros(E.shape[0])
...
def remove_corr_genes(E, gene_list, exclude_corr_genes_list, test_gene_idx, min_corr=0.1):
    ...
    for iSet in range(len(seed_ix_list)):
        seed_ix = seed_ix_list[iSet][E[:, seed_ix_list[iSet]].sum(
            axis=0).A.squeeze() > 0]
        ...
        for iG in range(len(c)):
            c[iG], _ = scipy.stats.pearsonr(
                tmp, E[:, test_gene_idx[iG]].A.squeeze())

        exclude_ix.extend([test_gene_idx[i] for i in range(
            len(test_gene_idx)) if (c[i]) >= min_corr])
...
def tot_counts_norm(E, exclude_dominant_frac=1, included=[], target_mean=0):
    ...
    if len(included) == 0:
        if exclude_dominant_frac == 1:
            ...
            wtmp.setdiag(1. / tots)
            included = np.asarray(
                ~(((wtmp * E) > exclude_dominant_frac).sum(axis=0) > 0))[0, :]
...
def preprocess_and_pca(E, total_counts_normalize=True, norm_exclude_abundant_gene_frac=1, min_counts=3, min_cells=5, min_vscore_pctl=85, gene_filter=None, num_pc=50, sparse_pca=False, show_vscore_plot=False):
    ...
    if total_counts_normalize:
        ...
        E = tot_counts_norm(
            E, exclude_dominant_frac=norm_exclude_abundant_gene_frac)[0]

    if gene_filter is None:
        # print 'Finding highly variable genes'
        gene_filter = filter_genes(E, min_vscore_pctl=min_vscore_pctl, min_counts=min_counts,
    ...
    if approx:
        ...
        if dist_metric == 'cosine':
            nbrs = NearestNeighbors(
                n_neighbors=k, metric=dist_metric, algorithm='brute').fit(X)
        else:
...
def write_graph(filename, n_nodes, edges):
    ...
    edges = [{'source': int(i), 'target': int(j), 'distance': 0}
             for i, j in edges]
...
def get_color_stats_custom(color_stats, custom_colors):
    for k, v in custom_colors.items():
        color_stats[k] = (np.mean(v), np.std(v), np.min(v),
                          np.max(v), np.percentile(v, 99))
    return color_stats


def save_color_stats(filename, color_stats):
    with open(filename, 'w') as f:
        f.write(json.dumps(color_stats, indent=4,
                           sort_keys=True))
...
def save_spring_dir_sparse_hdf5(E, gene_list, project_directory, edges, custom_colors={}, cell_groupings={}):
    ...
    write_color_tracks(custom_colors, project_directory +
    ...
    categorical_coloring_data = build_categ_colors(
        categorical_coloring_data, cell_groupings)
    save_cell_groupings(
        project_directory+'categorical_coloring_data.json', categorical_coloring_data)
...
def make_spring_subplot(E, gene_list, save_path, base_ix=None, normalize=True, exclude_dominant_frac=1.0, min_counts=3, min_cells=5, min_vscore_pctl=75, show_vscore_plot=False, exclude_gene_names=None, num_pc=30, sparse_pca=False, pca_norm=True, k_neigh=4, cell_groupings={}, num_force_iter=100, output_spring=True, precomputed_pca=None, gene_filter=None, custom_colors={}, exclude_corr_genes_list=None, exclude_corr_genes_minCorr=0.2, dist_metric='euclidean', use_approxnn=False, run_doub_detector=False, dd_k=50, dd_frac=5, dd_approx=True, tot_counts_final=None):
    ...
    if precomputed_pca is None:
        if gene_filter is None:
            ...
            if (min_counts > 0) or (min_cells > 0) or (min_vscore_pctl > 0):
                gene_filter = filter_genes(E, base_ix, min_vscore_pctl=min_vscore_pctl,
                                           min_counts=min_counts, min_cells=min_cells, show_vscore_plot=show_vscore_plot)
            else:
            ...
            if not exclude_corr_genes_list is None:
                gene_filter = remove_corr_genes(
                    E, gene_list, exclude_corr_genes_list, gene_filter, min_corr=exclude_corr_genes_minCorr)
                if len(gene_filter) == 0:
                    return 'Error: No genes passed filter'

            # Remove user-excluded genes from consideration
            if not exclude_gene_names is None:
                keep_ix = np.array([ii for ii, gix in enumerate(
                    gene_filter) if gene_list[gix] not in exclude_gene_names])
    ...
    links, knn_graph = get_knn_graph(
        Epca, k=k_neigh, dist_metric=dist_metric, approx=use_approxnn)
    ...
    if run_doub_detector:
        ...
        doub_score, doub_score_full, doub_labels = woublet.detect_doublets(
            [], counts=tot_counts_final, doub_frac=dd_frac, k=dd_k, use_approxnn=dd_approx, precomputed_pca=Epca)
